...
export async function performFileTransferTest(
  ...
  const transferCompletionTimeout = Math.max(
    PW_TIMEOUT * timeoutMultiplier * 3,
  ...
  try {
    await expect(senderPage.locator(senderStatusSelector)).toHaveText(/Completed/, {
      timeout: transferCompletionTimeout
    });
    ...
    const currentStatus = await senderPage
      .locator(senderStatusSelector)
      .textContent({ timeout: 2000 })
      .catch(() => 'Status not found or timed out');
    console.error(
      `Failed to find "Completed" status or links for sender ${senderName}. Current status: "${currentStatus}". Error:`,
      e
    );
    throw new Error(
      `Sender UI verification failed for ${senderName}: Status did not become 'Completed' or links not found. Current status: ${currentStatus}`
    );
  }
  ...
  for (let i = 0; i < receiverPages.length; i++) {
    ...
    console.log(
      `Verifying on ${receiverName}: Waiting for file container [data-filename="${fileName}"]...`
    );
    ...
    const receiverDownloadLinkLocator = receiverFileContainer.locator(
    ...
    await fs
      .unlink(tempFilePath)
      .catch((err) => console.warn(`Could not delete temp file ${tempFilePath}: ${err}`));
  }
  console.log(
    `--- SUCCESS (Playwright): ${fileName} transfer from ${senderName} verified on ${receiverNames.join(', ')} ---`
  );
}
