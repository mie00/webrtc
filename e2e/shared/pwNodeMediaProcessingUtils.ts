...
export async function takeScreenshotAndRecognizeText(
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    console.log(
      `Attempt ${attempt}/${maxAttempts}: Taking screenshot (flip: ${flip}) and attempting OCR...`
    );
    try {
      ...
      console.log(
        ` Attempt ${attempt}: OCR attempt complete. Text: "${ocrResult.text}", Confidence: ${ocrResult.confidence}`
      );
      ...
      console.error(
        ` Attempt ${attempt}: Error during screenshot or OCR:`,
...
async function decodeQrCodeWithTimeout(
  bitmap: Bitmap,
  ...
  const timeoutPromise = new Promise<null>((_, reject) => {
    setTimeout(
      () => reject(new Error(`QR code decoding timed out after ${timeoutMs}ms`)),
      timeoutMs
    );
  });
...
export async function takeScreenshotAndDecodeQR(
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    console.log(
      `Attempt ${attempt}/${maxAttempts}: Taking screenshot (flip: ${flip}) and attempting to decode QR code...`
    );
    try {
      let screenshotBuffer: Buffer;
      if (screenshotElementSelector) {
        ...
        screenshotBuffer = await element.screenshot({ type: 'png' });
        console.log(
          ` Attempt ${attempt}: Screenshot of element ${screenshotElementSelector} taken, buffer size: ${screenshotBuffer.length}`
        );
      } else {
        screenshotBuffer = await page.screenshot({ type: 'png' });
        console.log(
          ` Attempt ${attempt}: Full page screenshot taken, buffer size: ${screenshotBuffer.length}`
        );
      }
      ...
      console.log(
        ` Attempt ${attempt}: QR Code not found or could not be decoded (result: ${result}).`
      );
    } catch (error) {
      console.error(
        ` Attempt ${attempt}: Error during screenshot or QR decoding:`,
        (error as Error).message || error
      );
    }
...
export async function analyzeImageBufferForYuvNode(
  imageBuffer: Buffer,
  targetY: number = 128,
  yTolerance: number = 0.3 // Matches analyzeImageForYuvAveragesInBrowser default
...
export async function extractFramesAndAnalyzeVideoFileNode(
  ...
  console.log(
    `NodeJS: Expected QR content (for context): "${expectedQrContent}", Analyze audio: ${analyzeAudio}, Frames to extract: ${numFramesToExtract}, Browser: ${browserName}`
  );
  ...
  try {
    ...
    console.log(
      `NodeJS: Using constant video duration for ${videoFilePath}: ${videoDurationSeconds}s (from DEFAULT_AUDIO_DURATION_SECONDS)`
    );
    ...
    if (numFramesToExtract > 0) {
      ...
      console.log(
        `NodeJS: Calculated timemarks for ${numFramesToExtract} frames: ${calculatedTimemarks}`
      );
    }
    ...
    if (numFramesToExtract <= 0) {
      ...
      console.log(
        `NodeJS: Extracting ${numFramesToExtract} frames from ${videoFilePath} to ${tempDir} using calculated timemarks.`
      );
      if (calculatedTimemarks.length === 0) {
        // This should not be reached if numFramesToExtract > 0 due to the duration check above.
        console.error(
          'NodeJS: No timemarks calculated for frame extraction despite numFramesToExtract > 0. This indicates an issue.'
        );
    ...
    if (isFirefoxCamera) {
      ...
      for (let i = 1; i <= numFramesToExtract; i++) {
        ...
        console.log(
          `NodeJS: Frame ${i - 1} YUV analysis complete. ` +
      ...
      for (let i = 1; i <= numFramesToExtract; i++) {
        ...
        if (!(await fs.pathExists(framePath))) {
          ...
          result.ocrFramesAnalysis.push({
            frameIndex: i - 1,
            ocrResult: { text: null, confidence: 0, error: 'Frame not found' }
          });
          continue;
        }
        ...
        console.log(
          `NodeJS: Frame ${i - 1} OCR analysis complete. Text: "${ocrScanResult.text}", Confidence: ${ocrScanResult.confidence}`
        );
      }
    } else {
      // Default to QR code analysis
      console.log(
        `NodeJS: Performing QR code analysis for frames (Browser: ${browserName}, Expected Content: ${expectedQrContent}).`
      );
      for (let i = 1; i <= numFramesToExtract; i++) {
        const framePath = path.join(tempDir, `frame-${i}.png`);
        if (!(await fs.pathExists(framePath))) {
        ...
        for (const crop of crops) {
          try {
            ...
            if (croppedQr) {
              const alreadyFound = frameQrResults.some(
                (existingQr) =>
                  existingQr.result === croppedQr.result &&
                  Math.abs(existingQr.points[0].x - (croppedQr.points[0].x + crop.x)) <
                    width * 0.1 &&
                  Math.abs(existingQr.points[0].y - (croppedQr.points[0].y + crop.y)) < height * 0.1
              );
              if (!alreadyFound) {
                const adjustedPoints = croppedQr.points.map((p) => ({
                  x: p.x + crop.x,
                  y: p.y + crop.y
                }));
            ...
            console.warn(
              `NodeJS: Error decoding QR from cropped section: ${(cropError as Error).message}`
            );
          }
        }
        ...
        for (const r of frameQrResults) {
          if (
            !uniqueFrameQrResults.some(
              (uq) => uq.result === r.result && Math.abs(uq.points[0].x - r.points[0].x) < 10
            )
          ) {
        ...
        console.log(
          `NodeJS: Frame ${i - 1} yielded ${uniqueFrameQrResults.length} unique QR codes.`
        );
      }
    }
    ...
    if (analyzeAudio) {
      ...
      if ((await fs.pathExists(tempAudioPath)) && (await fs.stat(tempAudioPath)).size > 1024) {
        // Basic check for non-empty audio
        ...
        console.log(
          `NodeJS: Audio volume detection - Mean: ${meanVolume} dB, Max: ${maxVolume} dB`
        );
    ...
    console.error(
      `NodeJS: Error in extractFramesAndAnalyzeVideoFileNode: ${(error as Error).message || String(error)}`
    );
    ...
    try {
      ...
      console.error(
        `NodeJS: Error cleaning up temp directory ${tempDir}: ${(cleanupError as Error).message}`
      );
    }
  }
