...
import { analyzeAudioInBrowser, type AudioAnalysisResult } from './pwBrowserMediaUtils'; // Use Playwright version, YuvAnalysisResult and its function removed
import {
...
async function verifyAudioStreamOnPagePw(
  page: PlaywrightPage,
  pageName: string,
  expectedToPlay: boolean = true,
  isWatchTestAudio: boolean = false
): Promise<void> {
  console.log(
    `${pageName}: Verifying audio stream (expected: ${expectedToPlay ? 'playing' : 'silent/no source'}, type: ${isWatchTestAudio ? 'watch-test (chirp)' : 'mic (chirp/sine)'})...`
  );

  if (expectedToPlay && !isWatchTestAudio) {
    // This is a mic test, check for remote stream container visibility before analyzing
    console.log(
      `${pageName}: Mic test, ensuring remote stream container is visible before audio analysis...`
    );
    await expect(page.locator(REMOTE_VIDEO_CONTAINER_SELECTOR)).toBeVisible({
      timeout: getEffectiveTimeout(page)
    });
  ...
  const audioResult: AudioAnalysisResult = await page.evaluate(
  ...
  if (expectedToPlay) {
    ...
    if (isWatchTestAudio) {
      ...
      console.log(
        `${pageName}: Watch test audio chirp verified (Found ${uniqueFreqs.size} unique non-null frequencies, expected >1).`
      );
    } else {
      // Mic test audio
      if (browserName === 'chromium') {
        ...
        expect(uniqueFreqs.size).toBeGreaterThan(1);
        console.log(
          `${pageName}: Chromium mic audio chirp verified (Found ${uniqueFreqs.size} unique non-null frequencies, expected >1).`
        );
        ...
        console.log(
          `${pageName}: ${browserName} mic audio verified (Found ${uniqueFreqs.size} unique non-null frequencies, expected >=1 for sine wave/simple audio).`
        );
      } else {
        ...
        expect(uniqueFreqs.size).toBeGreaterThanOrEqual(1);
        console.log(
          `${pageName}: Mic audio verified for ${browserName || 'unknown browser'} (Found ${uniqueFreqs.size} unique non-null frequencies, expected >=1).`
        );
      }
    }
  } else {
    // For both Firefox and other browsers, if no audio is expected, verify silence.
    const isActiveAudio = audioResult.frequencies.some(
      (f) => f !== null && f > (analysisOptions.silenceThresholdDb || -80)
    );
...
async function verifyVideoStreamOnPagePw(
  page: PlaywrightPage,
  pageName: string,
  ...
  const browserName = page.context().browser()?.browserType().name();
  const isWebKitCamera =
    browserName === 'webkit' && expectedQrContent === CAMERA_TEST_QR_CONTENT_PW;
  const isFirefoxCamera =
    browserName === 'firefox' && expectedQrContent === CAMERA_TEST_QR_CONTENT_PW; // Watch tests use QR

  if (isFirefoxCamera) {
    console.log(
      `${pageName}: Verifying video stream (Firefox - Mid-Luminance YCbCr Dynamic Check) from element "${videoElementSelector}"...`
    );
    const numScreenshots = 3;
    const collectedMidLuminanceChroma: { cb: number | null; cr: number | null }[] = [];
    const minPercentageOfMidLuminancePixels = 0.2; // Expect at least 20% of pixels to be in Y tolerance.

    for (let i = 0; i < numScreenshots; i++) {
      await page
        .locator(videoElementSelector)
        .waitFor({ state: 'visible', timeout: getEffectiveTimeout(page) });
      if (i > 0)
        await page.waitForTimeout(1000); // Wait for potential change in video
      ...
      const analysisResult: YuvAnalysisResult =
        await analyzeImageBufferForYuvNode(screenshotBuffer);

      console.log(
        `${pageName}: Screenshot ${i + 1} Mid-Luminance (Y=${analysisResult.midLuminanceYValue} +/-${analysisResult.yTolerancePercentage * 100}%) Analysis: ` +
      ...
      expect(
        ...
        `Error in YCbCr analysis: ${analysisResult.error}`
      ).toBeUndefined();
      expect(analysisResult.percentageOfPixelsInYTolerance).toBeGreaterThanOrEqual(
        minPercentageOfMidLuminancePixels
      );
    ...
    const uniqueCbCrPairs = new Set(
      collectedMidLuminanceChroma.map(
        (chroma) => `${chroma.cb?.toFixed(1)},${chroma.cr?.toFixed(1)}`
      )
    );

    expect(uniqueCbCrPairs.size).toBeGreaterThan(1);
    console.log(
      `${pageName}: Video Cb/Cr (mid-lum) combination change verified (${uniqueCbCrPairs.size} unique avg (Cb,Cr) pairs: ${Array.from(uniqueCbCrPairs).join('; ')}).`
    );
  } else if (isWebKitCamera) {
    console.log(
      `${pageName}: Verifying video stream (WebKit OCR: timestamp format HH:MM:SS.mmm) from element "${videoElementSelector}"...`
    );
    ...
    for (let i = 0; i < maxAttempts && foundTimestamps.size < 2; i++) {
      await page
        ...
        .waitFor({ state: 'visible', timeout: getEffectiveTimeout(page) });
      if (i > 0)
        await page.waitForTimeout(1000); // Wait for video to change
      else await page.waitForTimeout(500); // Initial wait

      const ocrResult: OcrResult | null = await takeScreenshotAndRecognizeText(
        page,
        videoElementSelector,
        1,
        0,
        flip
      ); // 1 attempt, no extra delay here
      console.log(
        `${pageName}: Screenshot ${i + 1}/${maxAttempts} for OCR (flip: ${flip}). Result: ${JSON.stringify(ocrResult)}`
      );
      ...
      if (ocrResult && ocrResult.text) {
        ...
        if (match && match[0]) {
          ...
          console.log(
            `${pageName}: Found timestamp "${match[0]}" (Confidence: ${ocrResult.confidence}). Total unique: ${foundTimestamps.size}`
          );
        }
      }
    }
    expect(
      foundTimestamps.size,
      `${pageName}: Expected to find at least 2 different timestamps in video stream, found ${foundTimestamps.size}. Timestamps: ${Array.from(foundTimestamps).join(', ')}`
    ).toBeGreaterThanOrEqual(2);
    console.log(
      `${pageName}: WebKit video OCR verification successful. Found ${foundTimestamps.size} unique timestamps.`
    );
  } else {
    // Default to QR code verification (Chromium camera, all Watch tests)
    console.log(
      `${pageName}: Verifying video stream (QR content: "${expectedQrContent}") from element "${videoElementSelector}"...`
    );
    ...
    for (let i = 0; i < numScreenshots; i++) {
      ...
      console.log(
        `${pageName}: Screenshot ${i + 1}/${numScreenshots} taken for QR check (flip: ${flip}).`
      );
      ...
      console.log(
        `${pageName}: QR code squareness verified (Width: ${qrWidth.toFixed(2)}, Height: ${qrHeight.toFixed(2)}).`
      );
    ...
    console.log(
      `${pageName}: Video QR movement verified (${uniqueXCoords.size} unique X positions).`
    );
  }
}
...
export async function performMicTestPw(
  ...
  console.log(
    `--- Starting Mic Test (Playwright): ${sender.name} sends to ${receivers.map((r) => r.name).join(', ')} ---`
  );
  ...
  await expect(
    sender.page.locator(`${TOGGLE_AUDIO_BUTTON_SELECTOR}[class*="bg-blue-600"]`)
  ...
  if (checkSenderMutedState) {
    console.log(
      `${sender.name}: Verifying its own audio state (expected: no playable chirp for self-analysis).`
    );
    const analysisResultA: AudioAnalysisResult = await sender.page.evaluate(analyzeAudioInBrowser, {
      analysisType: 'amplitude' as const,
    ...
    console.log(
      `${sender.name}: Own audio state verified (no suitable source found for self-analysis).`
    );
  }
  ...
  await audioButton.click();
  await expect(
    sender.page.locator(`${TOGGLE_AUDIO_BUTTON_SELECTOR}:not([class*="bg-blue-600"])`)
  ).toBeVisible({ timeout: getEffectiveTimeout(sender.page) });
...
export async function performCameraTestPw(
  ...
  console.log(
    `--- Starting Camera Test (Playwright): ${sender.name} sends to ${receivers.map((r) => r.name).join(', ')} ---`
  );
  ...
  await expect(
    sender.page.locator(`${TOGGLE_VIDEO_BUTTON_SELECTOR}[class*="bg-blue-600"]`)
  ).toBeVisible({ timeout: getEffectiveTimeout(sender.page) });
  console.log(`${sender.name}: Video button ON. Waiting for stream propagation...`);
  await sender.page.waitForTimeout(2000);

  for (const receiver of receivers) {
    await expect(receiver.page.locator(REMOTE_VIDEO_ELEMENT_SELECTOR)).toBeVisible({
      timeout: getEffectiveTimeout(receiver.page, 2)
    });
    console.log(`${receiver.name}: Remote video element found.`);
    await verifyVideoStreamOnPagePw(
      receiver.page,
      receiver.name,
      REMOTE_VIDEO_ELEMENT_SELECTOR,
      CAMERA_TEST_QR_CONTENT_PW
    );
  }
  ...
  await videoButton.click();
  await expect(
    sender.page.locator(`${TOGGLE_VIDEO_BUTTON_SELECTOR}:not([class*="bg-blue-600"])`)
  ).toBeVisible({ timeout: getEffectiveTimeout(sender.page) });
...
export async function performWatchTestPw(
  ...
  console.log(
    `--- Starting Watch Test (Playwright): ${sender.name} sends to ${receivers.map((r) => r.name).join(', ')} ---`
  );
  ...
  await verifyVideoStreamOnPagePw(
    sender.page,
    sender.name,
    LOCAL_VIDEO_ELEMENT_SELECTOR_FILE,
    WATCH_TEST_QR_CONTENT_PW
  );
  ...
  await expect(
    sender.page.locator(`${SHARE_VIDEO_BUTTON_SELECTOR}:not([class*="bg-blue-600"])`)
  ).toBeVisible({ timeout: getEffectiveTimeout(sender.page) });
  console.log(`${sender.name}: "Share Video" button indicates video sharing stopped.`);
  await expect(sender.page.locator(LOCAL_VIDEO_CONTAINER_SELECTOR_FILE)).toBeHidden({
    timeout: getEffectiveTimeout(sender.page)
  });
...
export async function performCombinedMediaTestPw(
  ...
  console.log(
    `--- Starting Combined Media Test (Playwright): ${sender.name} sends to ${receivers.map((r) => r.name).join(', ')}, order: ${order} ---`
  );
  ...
  if (order === 'audioFirst') {
    // Activated A then V. Turn off V then A.
    console.log(`${sender.name}: Clicking video button to turn OFF.`);
    await videoButton.click();
    await expect(
      sender.page.locator(`${TOGGLE_VIDEO_BUTTON_SELECTOR}:not([class*="bg-blue-600"])`)
    ...
    // videoFirst. Activated V then A. Turn off A then V.
    console.log(`${sender.name}: Clicking audio button to turn OFF.`);
    await audioButton.click();
    await expect(
      sender.page.locator(`${TOGGLE_AUDIO_BUTTON_SELECTOR}:not([class*="bg-blue-600"])`)
...
async function verifyVideoFilePw(
  ...
  console.log(
    `${pageName}: Expected QR/Source type (content: "${expectedQrContent}"), Expected Audio: ${expectedAudio}, Browser: ${browserName}`
  );
  ...
  expect(
    analysisResult.error,
    `Error during video file analysis: ${analysisResult.error}`
  ).toBeUndefined();

  const isFirefoxCamera =
    browserName === 'firefox' && expectedQrContent === CAMERA_TEST_QR_CONTENT_PW;
  const isWebKitCamera =
    browserName === 'webkit' && expectedQrContent === CAMERA_TEST_QR_CONTENT_PW;
  ...
  if (isFirefoxCamera) {
    ...
    expect(
      analysisResult.yuvFramesAnalysis,
      `${pageName}: yuvFramesAnalysis should be defined for Firefox camera.`
    ).toBeDefined();
    expect(
      analysisResult.yuvFramesAnalysis!.length,
      `${pageName}: Expected ${numFramesToAnalyze} YUV frames.`
    ).toBe(numFramesToAnalyze);
    ...
    analysisResult.yuvFramesAnalysis!.forEach((yuvResult, index) => {
      console.log(
        `${pageName}: Frame ${index} YUV Analysis: ` +
      ...
      expect(
        yuvResult.error,
        `Error in YUV analysis for frame ${index}: ${yuvResult.error}`
      ).toBeUndefined();
      expect(yuvResult.percentageOfPixelsInYTolerance).toBeGreaterThanOrEqual(
        minPercentageOfMidLuminancePixels
      );
    ...
    console.log(
      `${pageName}: Video YUV dynamism verified (${uniqueCbCrPairs.size} unique avg (Cb,Cr) pairs from ${numFramesToAnalyze} frames).`
    );
  } else if (isWebKitCamera) {
    // --- OCR Verification for WebKit Camera Recording ---
    console.log(
      `${pageName}: Performing OCR verification for WebKit camera recording (timestamp format HH:MM:SS.mmm).`
    );
    expect(
      analysisResult.ocrFramesAnalysis,
      `${pageName}: ocrFramesAnalysis should be defined for WebKit camera.`
    ).toBeDefined();
    expect(
      analysisResult.ocrFramesAnalysis!.length,
      `${pageName}: Expected ${numFramesToAnalyze} OCR frames.`
    ).toBe(numFramesToAnalyze);
    ...
    analysisResult.ocrFramesAnalysis!.forEach((ocrFrame, index) => {
      console.log(
        `${pageName}: Frame ${index} OCR Analysis: Text: "${ocrFrame.ocrResult?.text}", Confidence: ${ocrFrame.ocrResult?.confidence}`
      );
      expect(
        ocrFrame.ocrResult?.error,
        `Error in OCR analysis for frame ${index}: ${ocrFrame.ocrResult?.error}`
      ).toBeUndefined();
      ...
      if (ocrFrame.ocrResult?.text) {
        ...
        if (match && match[0]) {
          foundTimestamps.add(match[0]);
          console.log(
            `${pageName}: Found timestamp "${match[0]}" in frame ${index} (Confidence: ${ocrFrame.ocrResult.confidence}). Total unique: ${foundTimestamps.size}`
          );
        }
      }
    });
    ...
    expect(
      foundTimestamps.size,
      `${pageName}: Expected to find at least 2 different timestamps in recorded video OCR analysis, found ${foundTimestamps.size}. Timestamps: ${Array.from(foundTimestamps).join(', ')}`
    ).toBeGreaterThanOrEqual(2);
    console.log(
      `${pageName}: WebKit recorded video OCR verification successful. Found ${foundTimestamps.size} unique timestamps.`
    );
  } else {
    // Default to QR Code Verification (Chromium Camera or other QR-based tests like Watch)
    console.log(
      `${pageName}: Performing QR code verification (Browser: ${browserName}, Expected Content: ${expectedQrContent}).`
    );
    const allFoundQrs: {
      result: string;
      points: { x: number; y: number }[];
    ...
    analysisResult.framesAnalysis.forEach((frameAnalysis) => {
      ...
      frameAnalysis.qrResults.forEach((qr) => {
        ...
        const qrWidth = maxX - minX;
        const qrHeight = maxY - minY;
        const tolerance = Math.min(qrWidth, qrHeight) * 0.2; // 20% tolerance
        expect(Math.abs(qrWidth - qrHeight)).toBeLessThanOrEqual(tolerance); // Verify squareness
      });
    });

    console.log(
      `${pageName}: Found ${totalQrDetections} QR code detections across ${analysisResult.framesAnalysis.length} analyzed frames.`
    );
    ...
    if (expectedQrContent === CAMERA_TEST_QR_CONTENT_PW) {
      // Only apply stricter check for camera test recordings
    ...
    if (allFoundQrs.length > 1) {
      ...
      console.warn(
        `${pageName}: Only one QR code instance found across multiple frames. Movement not robustly verified.`
      );
  ...
  if (expectedAudio) {
    ...
    const audioActivityDetected = analysisResult.audioAnalysis!.frequencies.some(
      (f) => f !== null && f > -50
    ); // Using -50dB as threshold from node analysis
    expect(
      audioActivityDetected,
      `${pageName}: Expected audio activity to be detected in the recording.`
    ).toBe(true);

    const uniqueFreqs = new Set(
      analysisResult.audioAnalysis!.frequencies.filter((f) => f !== null)
    );
    ...
    if (isWatchTestRecording) {
      ...
      // Mic test recording (from CAMERA_TEST_QR_CONTENT_PW context)
      if (browserName === 'chromium') {
    ...
    console.log(
      `${pageName}: Audio presence verified (Found ${uniqueFreqs.size} unique 'frequency' indicators/levels, expected ${conditionDescription}).`
    );
  } else {
    if (analysisResult.audioAnalysis) {
      // Audio might not have been analyzed if expectedAudio was false from start
      const audioActivityDetected = analysisResult.audioAnalysis.frequencies.some(
        (f) => f !== null && f > -50
      );
      ...
      console.log(
        `${pageName}: Audio analysis was not performed (as expectedAudio=false), considered silent.`
      );
    }
  }
...
export async function performRecordingTestPw(
  ...
  console.log(
    `--- Starting Recording Test (Playwright): ${pageInfoA.name} and ${pageInfoB.name} ---`
  );
  ...
  await expect(
    pageInfoA.page.locator(`${TOGGLE_VIDEO_BUTTON_SELECTOR}[class*="bg-blue-600"]`)
  ).toBeVisible({ timeout: getEffectiveTimeout(pageInfoA.page) });
  ...
  await expect(
    pageInfoB.page.locator(`${TOGGLE_VIDEO_BUTTON_SELECTOR}[class*="bg-blue-600"]`)
  ).toBeVisible({ timeout: getEffectiveTimeout(pageInfoB.page) });
  ...
  await expect(
    pageInfoA.page.locator(`${TOGGLE_AUDIO_BUTTON_SELECTOR}[class*="bg-blue-600"]`)
  ).toBeVisible({ timeout: getEffectiveTimeout(pageInfoA.page) });
  ...
  await verifyVideoStreamOnPagePw(
    pageInfoA.page,
    `${pageInfoA.name} (local view)`,
    LOCAL_VIDEO_ELEMENT_SELECTOR_CAMERA,
    CAMERA_TEST_QR_CONTENT_PW
  );
  console.log(`${pageInfoA.name}: Verifying remote camera stream from ${pageInfoB.name}.`);
  await verifyVideoStreamOnPagePw(
    pageInfoA.page,
    `${pageInfoA.name} (remote view of ${pageInfoB.name})`,
    REMOTE_VIDEO_ELEMENT_SELECTOR,
    CAMERA_TEST_QR_CONTENT_PW
  );
  ...
  await verifyVideoStreamOnPagePw(
    pageInfoB.page,
    `${pageInfoB.name} (remote view of ${pageInfoA.name})`,
    REMOTE_VIDEO_ELEMENT_SELECTOR,
    CAMERA_TEST_QR_CONTENT_PW
  );
  ...
  await expect(
    pageInfoA.page.locator(`${RECORD_BUTTON_SELECTOR}[class*="bg-red-600"]`)
  ).toBeVisible({ timeout: getEffectiveTimeout(pageInfoA.page) });
  ...
  await expect(
    pageInfoB.page.locator(`${RECORD_BUTTON_SELECTOR}[class*="bg-red-600"]`)
  ).toBeVisible({ timeout: getEffectiveTimeout(pageInfoB.page) });
  ...
  const downloadPromiseA = pageInfoA.page.waitForEvent('download', {
    timeout: getEffectiveTimeout(pageInfoA.page, 2)
  });
  await recordButtonA.click();
  await expect(
    pageInfoA.page.locator(`${RECORD_BUTTON_SELECTOR}:not([class*="bg-red-600"])`)
  ).toBeVisible({ timeout: getEffectiveTimeout(pageInfoA.page) });
  const downloadA = await downloadPromiseA;
  const filePathA = path.join(
    MEDIA_SETUP_DIR_PW,
    `recording_${pageInfoA.name.replace(' ', '_')}_${Date.now()}.webm`
  );
  ...
  const downloadPromiseB = pageInfoB.page.waitForEvent('download', {
    timeout: getEffectiveTimeout(pageInfoB.page, 2)
  });
  await recordButtonB.click();
  await expect(
    pageInfoB.page.locator(`${RECORD_BUTTON_SELECTOR}:not([class*="bg-red-600"])`)
  ).toBeVisible({ timeout: getEffectiveTimeout(pageInfoB.page) });
  const downloadB = await downloadPromiseB;
  const filePathB = path.join(
    MEDIA_SETUP_DIR_PW,
    `recording_${pageInfoB.name.replace(' ', '_')}_${Date.now()}.webm`
  );
  ...
  await verifyVideoFilePw(
    filePathA,
    CAMERA_TEST_QR_CONTENT_PW,
    true,
    `${pageInfoA.name} recording`,
    browserNameA
  );
