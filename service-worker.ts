...
(self as unknown as ServiceWorkerGlobalScope).addEventListener('fetch', (event: FetchEvent) => {
  ...
  if (event.resultingClientId && host) {
        ...
        new Promise<Response>((resolve, _reject) => {
          sw.handlers[id] = (data, err) => {
            ...
            try {
              ...
              resolve(
                new Response(arrayBuffer as ArrayBuffer, {
...
(self as unknown as ServiceWorkerGlobalScope).addEventListener(
  'message',
  function (event: ExtendableMessageEvent) {
    ...
    switch (event.data.type) {
      case 'host':
        ...
        sw.clientId =
          event.source && 'id' in event.source ? (event.source as Client).id : undefined;
        break;
      case 'response':
