...
vi.mock('./lib/webrtc/WebRTCApp.js', () => {
  console.log('[Mock Factory] Defining MockedWebRTCApp. mockWebRTCAppCleanup is named:', mockWebRTCAppCleanup.getMockName());
  class MockedWebRTCApp {
    ...
    constructor(config?: any) {
      this.config = config;
      console.log('[MockedWebRTCApp constructor] Assigning this.cleanup. mockWebRTCAppCleanup is named:', mockWebRTCAppCleanup.getMockName());
      ...
      console.log('[MockedWebRTCApp constructor] this.cleanup is now named:', this.cleanup.getMockName());
      console.log('[MockedWebRTCApp constructor] Is this.cleanup === mockWebRTCAppCleanup?', this.cleanup === mockWebRTCAppCleanup);
    }
...
describe('main.ts', () => {
  ...
  it('should add beforeunload event listener for cleanup', async () => {
    ...
    if (webRTCAppInstance && typeof webRTCAppInstance.cleanup === 'function') {
      console.log('[Test] webRTCAppInstance.cleanup is the spy:', webRTCAppInstance.cleanup === mockWebRTCAppCleanup);
