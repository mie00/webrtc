...
{#if show}
  <div
    id="copy-overlay"
    class="fixed inset-0 bg-black/75 flex justify-center items-center z-30 cursor-default"
    onclick={handleClose}
    role="button"
    tabindex="0"
  ...
  >
    <div class="bg-white p-4 rounded-md shadow-md text-center">
      ...
      {#if qrCodeDataURL}
        <div id="test-qr" class="flex justify-center" title={qrCodeUrl}>
          <img src={qrCodeDataURL} alt="QR Code" class="max-w-xs" />
        </div>
      {/if}
      ...
      <textarea
        ...
        value={copyText}
        id="test-copy"
        class="bg-gray-200 px-4 py-2 rounded-md break-all block mx-auto w-full max-w-md"
      ></textarea>
      {#if showCopyButton}
        <button
          id="test-copy-button"
      ...
      {#if showPasteText}
        <textarea
          ...
          class="bg-gray-200 px-4 py-2 rounded-md break-all block mt-2 mx-auto w-full max-w-md"
        ></textarea>
      {/if}
      {#if showAcceptButton}
        <button
