...
import '@testing-library/jest-dom';
  ...
  vi.mock('../stores/configStore.ts', async () => { // Added async here
  const { writable: actualWritable }: { writable: <T>(value: T, start?: any) => import('svelte/store').Writable<T> } = await vi.importActual('svelte/store'); // Import inside
  ...
  describe('ConfigOverlay.svelte', () => {
    it('renders general settings by default and allows tab switching', async () => {
      ...
      render(ConfigOverlay, { show: true, onclose: handleClose, onconfigUpdated: handleConfigUpdated });
