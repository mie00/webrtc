<script lang="ts">
  import { connectionStore } from '../lib/stores/connectionStore'; // getDirectClientState, getParticipantState are not directly used in template
...
<div class="border-b border-gray-300 pb-4 mb-4">
  ...
  <!-- Direct Connections -->
  {#each Object.entries($connectionStore.directClients || {}) as [cid, client] (cid)}
    {@const state = client.state}
    {@const iceState = client.iceState}
    ...
    {@const isConnecting = !isConnected && !isFailed && (state !== null || iceState !== null)}
    <!-- Show yellow if not connected/failed but trying -->
    {@const clientKeys = getKeysByCid(cid)}
    {@const userProfile = clientKeys?.userPublicKey
      ? getPeerProfile(clientKeys.userPublicKey)
      : undefined}
    {@const displayName = userProfile?.userName || cid}
    <div class="flex items-center space-x-2 mb-1">
      <div
        id="test-indicator-{cid}"
        ...
        title={`Direct: ${cid}\nState: ${state ?? 'N/A'}\nICE: ${iceState ?? 'N/A'}`}
      ></div>
      <p class="text-sm font-medium text-gray-700" title={`CID: ${cid}`}>
        {displayName}
  ...
  {#each Object.values($connectionStore.participants || {}) as participant (participant.cid)}
    ...
    {#if !($connectionStore.directClients || {})[participant.cid]}
      {@const relayClient = participant.relayCid
        ? ($connectionStore.directClients || {})[participant.relayCid]
        : undefined}
      {@const relayState = relayClient?.state}
      {@const relayIceState = relayClient?.iceState}
      {@const isRelayConnected = relayState === 'connected' && relayIceState === 'connected'}
      {@const isRelayFailed =
        !relayClient ||
        relayState === 'failed' ||
        relayIceState === 'failed' ||
        relayState === 'closed' ||
        relayIceState === 'closed' ||
        relayState === 'disconnected' ||
        relayIceState === 'disconnected'}
      {@const isRelayConnecting =
        relayClient &&
        !isRelayConnected &&
        !isRelayFailed &&
        (relayState !== null || relayIceState !== null)}
      {@const participantKeys = getKeysByCid(participant.cid)}
      {@const relayedUserProfile = participantKeys?.userPublicKey
        ? getPeerProfile(participantKeys.userPublicKey)
        : undefined}
      ...
      <div class="flex items-center space-x-2 mb-1 opacity-75">
        <div
          ...
          class:bg-gray-200={!relayClient ||
            (!isRelayConnected && !isRelayFailed && !isRelayConnecting)}
          title={`Relayed: ${participant.cid}\nVia: ${participant.relayCid}\nRelay State: ${relayState ?? 'N/A'}\nRelay ICE: ${relayIceState ?? 'N/A'}`}
        ></div>
        <p
          class="text-sm font-medium text-gray-500 truncate"
          title={`CID: ${participant.cid} (via ${participant.relayCid})`}
        >
