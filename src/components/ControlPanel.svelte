<script lang="ts">
  ...
  const combinedFeed = $derived(
    (() => {
      ...
      const transcriptionItems: FeedItem[] = ($transcriptionDisplayStore.segments || []).map(
        (seg, i) => ({
          id: `transcription-${seg.id || `${seg.utteranceId}-${i}`}`,
          type: 'transcription',
          sender: seg.speakerLabel,
    ...
    return feed.filter((item) => {
      const isLocal =
        item.type === 'chat'
          ...
          : item.type === 'file'
            ? !item.cid
            : false; // Files are local if no senderCid, transcriptions are more complex
      return !isLocal;
    }).length;
  }
  ...
  function openMediaCarousel(clickedItem: CarouselMediaItem) {
    ...
    const getPlayableMediaType = (type: string) => {
      // Local helper for this function
    ...
    for (const item of combinedFeed) {
      if (
        ...
        currentViewableMedia.push({
          ...
          transfer: item.transfer as FileTransfer & { url: string },
          cid: item.cid
        });
      }
    }
