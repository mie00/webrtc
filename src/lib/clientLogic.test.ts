...
describe('ClientLogic', () => {
  ...
  beforeEach(async () => { // Make beforeEach async
    ...
    const configStoreModule = await vi.importActual<typeof import('../stores/configStore')>('../stores/configStore');
    ...
    if (typeof actualDefaultConfig === 'undefined') {
      ...
      throw new Error("defaultConfig from vi.importActual is undefined. Ensure it is exported from configStore.ts.");
    }
  ...
  describe('setConfig', () => {
    ...
    describe('initialize', () => {
      ...
      it('should process an answer from URL params and broadcast it', async () => {
        ...
        expect(mockContext.broadcastManuallyEnteredAnswer).toHaveBeenCalledWith(offerParam, answerParam);
    ...
    describe('acceptHandler', () => {
      ...
      it('should use currentOfferCid from state if cidFromEvent is null', async () => {
        ...
        expect(mockSetRemoteDescription).toHaveBeenCalledWith({ type: "answer", sdp: decompressedAnswer + '\n' });
        expect(mockContext.setState).toHaveBeenCalledWith({ showCopyOverlay: false, initialOverlayShown: false });
      });
