...
import { getDirectClient, getAllClientCids, getAllDirectClients } from '../../stores/connectionStore';
...
describe('fileTransfer', () => {
  ...
  describe('setupFileChannel', () => {
    ...
    it('should handle empty file reception', () => {
      ...
      expect(addFileTransfer).toHaveBeenCalledWith(expect.objectContaining({
        name: 'empty.txt',
        size: 0,
    ...
    it('should not forward messages if other client dc_file is not open or does not exist', () => {
      ...
      const mockOtherClientClosedDc = { pc: new MockRTCPeerConnection(), dc_file: new MockRTCDataChannel('file-other') };
  ...
  describe('sendFile', () => {
    ...
    it('should add file transfer and attempt to read and send file', async () => {
      ...
      expect(updateFileTransfer).toHaveBeenCalledWith('1f9add3f0c9a5', { progress: 100, status: 'complete' });
    });
    ...
    it('should use dynamic SEND_CHUNK_SIZE based on SDP if available', async () => {
      ...
      (getMaxMessageSizeFromSdp as vi.Mock)
        .mockImplementation((sdp: string) => {
    ...
    it('should handle waitForBufferDrain when bufferedAmount is high', async () => {
      ...
      mockFile = new File([fileContent.buffer], 'large-file.bin', { type: 'application/octet-stream' });
      const mockReaderInstance = {
        readAsArrayBuffer: vi.fn(),
        onload: null as ((e: ProgressEvent<FileReader>) => void) | null,
      ...
      (splitArrayBuffer as vi.Mock).mockImplementation((buffer: ArrayBuffer, _chunkSize: number) => {
      ...
      mockClient.dc_file.send.mockImplementation(() => {
        ...
        if (bufferedAmountLowCallback && mockClient.dc_file.bufferedAmount > (0.125 * 1024 * 1024) / 2) {
           ...
           Promise.resolve().then(() => { // Ensure this runs after the current microtask
      ...
      expect(mockClient.dc_file.addEventListener).toHaveBeenCalledWith('bufferedamountlow', expect.any(Function));
    });
